...
def get_counter(file_target):
    ...
    return int(b[0])
#################################
def get_decision(val, default):
    counter = get_counter("decision")
    the_num = counter
    f = open("decision", 'w')
    if val > default:
        f.write("True\n")
...
def make_dir(name):
    ...
    os.mkdir(name)
###############################
def cp_log(directory):
    
    for i in os.listdir(directory):
        if os.path.isfile(directory + os.sep +i) and i[-4:]== ".log":
            shutil.copy2(i, str(get_counter("decision")-1))    
...
get_decision(max_val, conv)
make_dir(str(get_counter("decision")-1))
cp_log(".")
